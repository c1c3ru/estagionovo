...
class ErrorHandler {
  ...
  static voidhandle(
    ...
    BuildContext? context, // Contexto para mostrar UI de erro (SnackBar, Dialog)
  }) {
    logger.e(
  ...
  static AppFailure convertToAppFailure(dynamic error, {String? defaultMessage}) {
    if (error is AppFailure) {
      return error;
    } else if (error is supabase_auth.AuthException) {
      return AuthenticationFailure(message: error.message, originalException: error);
    } else if (error is PostgrestException) { // Exceção do banco de dados Supabase
      return SupabaseServerFailure(message: error.message ?? 'Erro no servidor Supabase', originalException: error);
    }
