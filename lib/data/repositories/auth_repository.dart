...
class AuthRepository implements IAuthRepository {
  ...
  UserEntity _mapSupabaseUserToUserEntity(supabase_auth.User supabaseUser) {
    return UserEntity(
      ...
      isActive:
          supabaseUser.userMetadata?['is_active'] as bool? ??
      ...
      if (authResponse.user != null) {
        ...
        return Left(
          AuthenticationFailure(
            message: 'Utilizador não retornado após o login.',
          ),
        );
      }
  ...
  Future<Either<AppFailure, UserEntity>> register(RegisterParams params) async {
    try {
      ...
      if (authResponse.user != null) {
        ...
        return Left(
          AuthenticationFailure(
            message: 'Utilizador não retornado após o registo.',
          ),
        );
      }
    } on supabase_auth.AuthException catch (e) {
  ...
  Future<Either<AppFailure, void>> logout() async {
    try {
      ...
      return const Right(
        null,
      ); // Sucesso é representado por Right(null) para void
    } on supabase_auth.AuthException catch (e) {
      return Left(
        AuthenticationFailure(message: e.message, originalException: e),
      );
  ...
  Future<Either<AppFailure, UserEntity>> updateUserProfile(
    ...
    try {
      ...
      final updatedSupabaseUser = await _authDatasource.updateSupabaseUser(
  ...
  Stream<UserEntity?> get authStateChanges {
    return _authDatasource.supabaseAuthStateChanges
        .map((supabaseAuthState) {
