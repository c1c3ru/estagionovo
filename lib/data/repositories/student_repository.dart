...
import 'package:supabase_flutter/supabase_flutter.dart' show PostgrestException; // Apenas para type checking da excepção
...
import '../models/enums.dart'; // Para os enums, se necessário no mapeamento
...
class StudentRepository implements IStudentRepository {
  ...
  @override
  Future<Either<AppFailure, StudentEntity>> getStudentDetails(String userId) async {
    try {
      final studentData = await _studentDatasource.getStudentDataById(userId);
      if (studentData != null) {
        return Right(_mapDataToStudentEntity(studentData));
      } else {
        return Left(NotFoundFailure(message: 'Perfil de estudante não encontrado para o ID: $userId'));
      }
  ...
  @override
  Future<Either<AppFailure, StudentEntity>> updateStudentProfile(UpdateStudentProfileParams params) async {
    try {
      ...
      if (params.registrationNumber != null) dataToUpdate['registration_number'] = params.registrationNumber;
      if (params.course != null) dataToUpdate['course'] = params.course;
      if (params.advisorName != null) dataToUpdate['advisor_name'] = params.advisorName;
      if (params.isMandatoryInternship != null) dataToUpdate['is_mandatory_internship'] = params.isMandatoryInternship;
      if (params.profilePictureUrl != null) dataToUpdate['profile_picture_url'] = params.profilePictureUrl;
      if (params.phoneNumber != null) dataToUpdate['phone_number'] = params.phoneNumber;
      // Novos campos adicionados
      if (params.birthDate != null) dataToUpdate['birth_date'] = params.birthDate!.toIso8601String().substring(0, 10);
      if (params.classShift != null) dataToUpdate['class_shift'] = params.classShift!.value;
      ...
      final updatedStudentData = await _studentDatasource.updateStudentData(params.studentId, dataToUpdate);
      return Right(_mapDataToStudentEntity(updatedStudentData));
    } on PostgrestException catch (e) {
      return Left(SupabaseServerFailure(message: 'Erro do Supabase ao atualizar perfil do estudante: ${e.message}', originalException: e));
    } on ServerException catch (e) {
      return Left(ServerFailure(message: e.message, originalException: e.originalException));
    } catch (e) {
      return Left(ServerFailure(message: 'Erro desconhecido ao atualizar perfil do estudante: ${e.toString()}', originalException: e));
    }
  }
  ...
  Future<Either<AppFailure, List<TimeLogEntity>>> getStudentTimeLogs({
    ...
    try {
      ...
      return Left(SupabaseServerFailure(message: 'Erro do Supabase ao obter registos de tempo: ${e.message}', originalException: e));
    } on ServerException catch (e) {
      return Left(ServerFailure(message: e.message, originalException: e.originalException));
    } catch (e) {
      return Left(ServerFailure(message: 'Erro desconhecido ao obter registos de tempo: ${e.toString()}', originalException: e));
    }
  }
  ...
  Future<Either<AppFailure, TimeLogEntity>> createTimeLog({
    ...
    try {
      final Map<String, dynamic> timeLogData = {
        ...
        if (checkOutTime != null) 'check_out_time': '${checkOutTime.hour.toString().padLeft(2, '0')}:${checkOutTime.minute.toString().padLeft(2, '0')}:00',
    ...
    try {
      final Map<String, dynamic> dataToUpdate = {
        ...
        if (timeLog.checkOutTime != null)
          'check_out_time': '${timeLog.checkOutTime!.hour.toString().padLeft(2, '0')}:${timeLog.checkOutTime!.minute.toString().padLeft(2, '0')}:00',
        // Outros campos como 'approved' seriam atualizados por um supervisor
      };

      // Recalcular hours_logged se checkOutTime for atualizado
      if (timeLog.checkOutTime != null) {
        final start = DateTime(timeLog.logDate.year, timeLog.logDate.month, timeLog.logDate.day, timeLog.checkInTime.hour, timeLog.checkInTime.minute);
        final end = DateTime(timeLog.logDate.year, timeLog.logDate.month, timeLog.logDate.day, timeLog.checkOutTime!.hour, timeLog.checkOutTime!.minute);
         if (end.isAfter(start)) {
          final durationInMinutes = end.difference(start).inMinutes;
          dataToUpdate['hours_logged'] = (durationInMinutes / 60.0);
        } else {
          dataToUpdate['hours_logged'] = timeLog.hoursLogged ?? 0.0; // Mantém o anterior ou 0 se inválido
        }
      }
      ...
      final updatedTimeLogData = await _timeLogDatasource.updateTimeLogData(timeLog.id, dataToUpdate);
  ...
  Future<Either<AppFailure, TimeLogEntity>> checkIn({
    ...
    try {
      ...
      final timeLogData = {
        ...
        'check_in_time': '${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')}:00',
        if (notes != null && notes.isNotEmpty) 'description': notes,
        'approved': false, // Check-ins geralmente não são aprovados imediatamente
      };
      final createdLogData = await _timeLogDatasource.createTimeLogData(timeLogData);
      return Right(_mapDataToTimeLogEntity(createdLogData));
    } on PostgrestException catch (e) {
      return Left(SupabaseServerFailure(message: 'Erro do Supabase ao fazer check-in: ${e.message}', originalException: e));
    } on ServerException catch (e) {
      return Left(ServerFailure(message: e.message, originalException: e.originalException));
    } catch (e) {
      return Left(ServerFailure(message: 'Erro desconhecido ao fazer check-in: ${e.toString()}', originalException: e));
    }
  }

  @override
  Future<Either<AppFailure, TimeLogEntity>> checkOut({
    required String studentId, // studentId não é usado diretamente para encontrar o log, mas pode ser para validação
    required String activeTimeLogId,
    String? description,
  }) async {
    try {
      // 1. Buscar o log de tempo ativo para obter o check_in_time e log_date
      final activeLogData = await _timeLogDatasource.getTimeLogDataById(activeTimeLogId);
      if (activeLogData == null) {
        return Left(NotFoundFailure(message: 'Registo de tempo ativo não encontrado para check-out.'));
      }
      ...
      if (checkInTimeStr == null) {
         return Left(ServerFailure(message: 'Dados de check-in inválidos no registo ativo.'));
      }
      final checkInParts = checkInTimeStr.split(':');
      final checkInTime = TimeOfDay(hour: int.parse(checkInParts[0]), minute: int.parse(checkInParts[1]));

      final now = DateTime.now();
      final checkOutTime = TimeOfDay(hour: now.hour, minute: now.minute);

      final Map<String, dynamic> dataToUpdate = {
        'check_out_time': '${checkOutTime.hour.toString().padLeft(2, '0')}:${checkOutTime.minute.toString().padLeft(2, '0')}:00',
      };
      if (description != null && description.isNotEmpty) {
        dataToUpdate['description'] = (activeLogData['description'] as String? ?? '') + (description.isNotEmpty ? '\nCheckout: $description' : '');
      }

      // Calcular hours_logged
      final startDateTime = DateTime(logDate.year, logDate.month, logDate.day, checkInTime.hour, checkInTime.minute);
      final endDateTime = DateTime(logDate.year, logDate.month, logDate.day, checkOutTime.hour, checkOutTime.minute);

      if (endDateTime.isAfter(startDateTime)) {
        final durationInMinutes = endDateTime.difference(startDateTime).inMinutes;
        dataToUpdate['hours_logged'] = (durationInMinutes / 60.0);
      } else {
        dataToUpdate['hours_logged'] = 0.0; // Ou manter o valor anterior se já existia
      }

      final updatedLogData = await _timeLogDatasource.updateTimeLogData(activeTimeLogId, dataToUpdate);
      return Right(_mapDataToTimeLogEntity(updatedLogData));
    } on PostgrestException catch (e) {
      return Left(SupabaseServerFailure(message: 'Erro do Supabase ao fazer check-out: ${e.message}', originalException: e));
    } on ServerException catch (e) {
      return Left(ServerFailure(message: e.message, originalException: e.originalException));
    } on NotFoundFailure catch(e){
      return Left(e);
    } catch (e) {
      return Left(ServerFailure(message: 'Erro desconhecido ao fazer check-out: ${e.toString()}', originalException: e));
    }
  }
}
