...
class SupervisorRepository implements ISupervisorRepository {
  ...
  SupervisorEntity _mapDataToSupervisorEntity(Map<String, dynamic> data) {
    return SupervisorEntity(
      ...
      siapeRegistration:
          '', // Assumindo que esta entidade é sempre um supervisor
    );
  }
  ...
  Future<Either<AppFailure, SupervisorEntity>> getSupervisorDetails(
    ...
    try {
      final supervisorData = await _supervisorDatasource.getSupervisorDataById(
        userId,
      );
  ...
  Future<Either<AppFailure, List<StudentEntity>>> getAllStudents(
    ...
    try {
      ...
      final studentEntities = studentsData
          .map((data) => _mapDataToStudentEntity(data))
          .toList();
  ...
  Future<Either<AppFailure, StudentEntity>> getStudentDetailsForSupervisor(
    ...
    try {
      final studentData = await _studentDatasource.getStudentDataById(
        studentId,
      );
      if (studentData != null) {
  ...
  Future<Either<AppFailure, StudentEntity>> createStudent(
    ...
    try {
      ...
      final Map<String, dynamic> dataToCreate = {
        ...
        if (studentData.internshipShift2 != null)
          'internship_shift_2': studentData.internshipShift2!.value,
        'birth_date': studentData.birthDate.toIso8601String().substring(0, 10),
        'contract_start_date': studentData.contractStartDate
            .toIso8601String()
            .substring(0, 10),
        'contract_end_date': studentData.contractEndDate
            .toIso8601String()
            .substring(0, 10),
      ...
      final createdStudentData = await _studentDatasource.createStudentData(
        dataToCreate,
      );
  ...
  Future<Either<AppFailure, StudentEntity>> updateStudentBySupervisor(
    ...
    try {
      final Map<String, dynamic> dataToUpdate = {
        ...
        'internship_shift_1': studentData.internshipShift1.value,
        'internship_shift_2':
            studentData.internshipShift2?.value, // Envia null se for null
        'birth_date': studentData.birthDate.toIso8601String().substring(0, 10),
        'contract_start_date': studentData.contractStartDate
            .toIso8601String()
            .substring(0, 10),
  ...
  Future<Either<AppFailure, List<ContractEntity>>> getAllContracts(
    ...
    try {
      ...
      final contractEntities = contractsData
          .map((data) => _mapDataToContractEntity(data))
          .toList();
  ...
  Future<Either<AppFailure, ContractEntity>> createContract(
    ...
    try {
      final Map<String, dynamic> dataToCreate = {
        ...
        'created_by':
            contractData.createdBy, // ID do auth.user que está a criar
      };
      final createdContractData = await _contractDatasource.createContractData(
        dataToCreate,
      );
