...
class TimeLogRepository implements ITimeLogRepository {
  ...
  Future<Either<AppFailure, TimeLogEntity>> getTimeLogById(
    ...
    try {
      final timeLogData = await _timeLogDatasource.getTimeLogDataById(
        timeLogId,
      );
      if (timeLogData != null) {
  ...
  Future<Either<AppFailure, TimeLogEntity>> updateTimeLog(
    ...
    try {
      ...
      if (params.logDate != null)
        dataToUpdate['log_date'] = params.logDate!.toIso8601String().substring(
          0,
          10,
        );
      if (params.checkInTime != null)
        dataToUpdate['check_in_time'] = _formatTimeOfDayForDb(
          params.checkInTime,
        );
      if (params.checkOutTime != null)
        dataToUpdate['check_out_time'] = _formatTimeOfDayForDb(
          params.checkOutTime,
        );
      ...
      if (finalCheckIn != null &&
        ...
        if (end.isAfter(start)) {
          ...
          dataToUpdate['hours_logged'] =
              0.0; // Ou null, ou manter o valor anterior
        }
      }
