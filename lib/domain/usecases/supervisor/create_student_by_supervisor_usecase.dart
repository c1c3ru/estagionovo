...
class CreateStudentBySupervisorUsecase {
  ...
  Future<Either<AppFailure, StudentEntity>> call(StudentEntity studentData) async {
    ...
    if (studentData.fullName.trim().isEmpty) {
      return Left(ValidationFailure('O nome completo do estudante é obrigatório.'));
    }
    if (studentData.registrationNumber.trim().isEmpty) {
      return Left(ValidationFailure('O número de matrícula do estudante é obrigatório.'));
    }
