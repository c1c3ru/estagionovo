...
class _StudentProfilePageState extends State<StudentProfilePage> {
  ...
  final _birthDateController = TextEditingController(); // Para exibir a data selecionada
  ...
  void initState() {
    ...
    if (currentAuthState is auth_state.AuthSuccess) {
      ...
      if (_currentUserId != null) {
        ...
        if (currentStudentState is StudentDashboardLoadSuccess && currentStudentState.student.id == _currentUserId) {
  ...
  void _saveProfileChanges() {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      _studentBloc.add(UpdateStudentProfileInfoEvent(
    ...
    return Scaffold(
      ...
      body: BlocConsumer<StudentBloc, StudentState>(
        ...
        builder: (context, state) {
          ...
          if (_currentStudent == null && _currentUserId != null && state is! StudentLoading) {
             _studentBloc.add(LoadStudentDashboardDataEvent(userId: _currentUserId!));
             return const LoadingIndicator();
          }
      ...
      child: ListView(
        ...
        children: <Widget>[
          ...
          if (_isEditMode) ...[
            ...
            _buildReadOnlyInfo(context, 'Nome Completo', student.fullName, icon: Icons.person_outline),
            _buildReadOnlyInfo(context, 'Nº de Matrícula', student.registrationNumber, icon: Icons.badge_outlined),
            _buildReadOnlyInfo(context, 'Curso', student.course, icon: Icons.school_outlined),
            _buildReadOnlyInfo(context, 'Orientador(a)', student.advisorName, icon: Icons.supervisor_account_outlined),
            _buildReadOnlyInfo(context, 'Telefone', student.phoneNumber ?? 'Não informado', icon: Icons.phone_outlined),
            _buildReadOnlyInfo(context, 'Email', _authBloc.state is auth_state.AuthSuccess ? (_authBloc.state as auth_state.AuthSuccess).user.email : 'Não informado', icon: Icons.email_outlined),
            _buildReadOnlyInfo(context, 'Data de Nascimento', DateFormat('dd/MM/yyyy').format(student.birthDate), icon: Icons.cake_outlined),
            _buildReadOnlyInfo(context, 'Estágio Obrigatório', student.isMandatoryInternship ? 'Sim' : 'Não', icon: Icons.star_border_outlined),
            _buildReadOnlyInfo(context, 'Turno das Aulas', student.classShift.displayName, icon: Icons.schedule_outlined),
            _buildReadOnlyInfo(context, 'Turno do Estágio 1', student.internshipShift1.displayName, icon: Icons.work_outline),
            if (student.internshipShift2 != null)
              _buildReadOnlyInfo(context, 'Turno do Estágio 2', student.internshipShift2!.displayName, icon: Icons.work_history_outlined),
            _buildReadOnlyInfo(context, 'Início do Contrato', DateFormat('dd/MM/yyyy').format(student.contractStartDate), icon: Icons.play_circle_outline),
            _buildReadOnlyInfo(context, 'Fim do Contrato', DateFormat('dd/MM/yyyy').format(student.contractEndDate), icon: Icons.stop_circle_outlined),
            _buildReadOnlyInfo(context, 'Horas Necessárias', '${student.totalHoursRequired.toStringAsFixed(1)}h', icon: Icons.hourglass_bottom_outlined),
            _buildReadOnlyInfo(context, 'Horas Completas', '${student.totalHoursCompleted.toStringAsFixed(1)}h', icon: Icons.hourglass_full_outlined),
  ...
  Widget _buildEditableFields(BuildContext context) {
    ...
    return Column(
      children: [
        Center( // Para a foto e URL no modo de edição
          child: Column(
            children: [
               CircleAvatar(
                radius: 50,
                ...
                child: _profilePictureUrlController.text.isEmpty
                    ? Icon(Icons.person_add_a_photo_outlined, size: 40, color: Theme.of(context).colorScheme.onPrimaryContainer)
                    : null,
              ),
              ...
              AppTextField(
                ...
                onChanged: (value) => setState(() {}), // Para atualizar a prévia da imagem
              ),
            ],
          )
        ),
        ...
        AppTextField(
          ...
          onTap: () async {
            final DateTime? picked = await showDatePicker(
              context: context,
              initialDate: _selectedBirthDate ?? DateTime.now().subtract(const Duration(days: 365 * 18)), // Ex: 18 anos atrás
        ...
        DropdownButtonFormField<ClassShift>(
          ...
          validator: (value) => value == null || value == ClassShift.unknown ? 'Selecione um turno válido' : null,
        ),
        const SizedBox(height: 16),
        // Estágio Obrigatório
        SwitchListTile(
  ...
  Widget _buildReadOnlyInfo(BuildContext context, String label, String value, {IconData? icon}) {
    final theme = Theme.of(context);
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 10.0), // Aumentado o padding vertical
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          if (icon != null) ...[
            Icon(icon, size: 22, color: theme.colorScheme.primary), // Ícone um pouco maior
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  label,
                  style: theme.textTheme.bodySmall?.copyWith(color: theme.hintColor, fontSize: 12), // Fonte menor para o label
                ),
                const SizedBox(height: 3), // Espaçamento ajustado
                Text(
                  value,
                  style: theme.textTheme.bodyLarge?.copyWith(fontSize: 15), // Fonte um pouco menor para o valor
                ),
              ],
            ),
          ),
