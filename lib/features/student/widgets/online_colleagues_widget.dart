  ...
  Future<void> _initializeAndFetchOnlineColleagues() async {
    ...
    try {
      ...
      final response = await _supabaseClient
          ...
          .neq(
            'id',
            _currentUserId ?? 'dummy_id_to_avoid_error_if_null',
      ...
      for (var studentData in response as List<dynamic>) {
        initialOnline.add(
          OnlineColleague.fromInitialQuery(studentData as Map<String, dynamic>),
        );
      }
      setState(() {
    ...
    if (_onlineColleagues.isEmpty) {
      return Card(
        ...
        child: const Padding(
          ...
          child: Center(child: Text('Nenhum colega online no momento.')),
        ),
      );
    }
