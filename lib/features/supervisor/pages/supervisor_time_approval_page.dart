...
class SupervisorTimeApprovalPage extends StatefulWidget {
  ...
  @override
  State<SupervisorTimeApprovalPage> createState() => _SupervisorTimeApprovalPageState();
}

class _SupervisorTimeApprovalPageState extends State<SupervisorTimeApprovalPage> {
  ...
  Future<void> _loadPendingApprovals() async {
    ...
    _supervisorBloc.add(const LoadAllTimeLogsForApprovalEvent(pendingOnly: true));
  }
  ...
  Future<String> _getStudentName(String studentId, SupervisorState currentState) async {
    ...
    if (currentState is SupervisorDashboardLoadSuccess) {
      try {
        final student = currentState.students.firstWhere((s) => s.id == studentId);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: const SupervisorAppDrawer(currentIndex: 2), // Ajuste o currentIndex conforme sua navegação
      bottomNavigationBar: const SupervisorBottomNavBar(currentIndex: 2), // Ajuste o currentIndex
      body: BlocConsumer<SupervisorBloc, SupervisorState>(
        ...
        listener: (context, state) {
          ...
          } else if (state is SupervisorOperationSuccess && (state.entity is TimeLogEntity || state.message.contains("Registo de tempo"))) {
            ScaffoldMessenger.of(context)
              ..hideCurrentSnackBar()
              ..showSnackBar(SnackBar(
                  content: Text(state.message),
        ...
        builder: (context, state) {
          if (state is SupervisorLoading && state is! SupervisorTimeLogsForApprovalLoadSuccess) {
             if (_supervisorBloc.state is! SupervisorTimeLogsForApprovalLoadSuccess) {
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return FutureBuilder<String>(
                    future: _getStudentName(log.studentId, state), // Passa o estado atual
                    builder: (context, snapshot) {
                      final studentName = snapshot.data ?? 'A carregar nome...';
          ...
          if (state is SupervisorOperationFailure && _supervisorBloc.state is! SupervisorTimeLogsForApprovalLoadSuccess) {
  ...
  Widget _buildTimeLogApprovalCard(BuildContext context, TimeLogEntity log, String studentName) {
    final theme = Theme.of(context);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow(context, Icons.login_outlined, 'Entrada:', checkInStr),
            if (log.checkOutTime != null)
              _buildDetailRow(context, Icons.logout_outlined, 'Saída:', checkOutStr),
            _buildDetailRow(context, Icons.hourglass_full_outlined, 'Horas Registadas:', hoursStr),
            if (log.description != null && log.description!.isNotEmpty)
              _buildDetailRow(context, Icons.notes_outlined, 'Descrição:', log.description!),

            const SizedBox(height: 12),
            // Botões de Ação
            if (!log.approved && _supervisorId != null) // Mostra botões apenas se não aprovado e supervisorId estiver disponível
              BlocBuilder<SupervisorBloc, SupervisorState>(
                bloc: _supervisorBloc, // Usa o BLoC da página
                builder: (context, state) {
                  bool isLoadingAction = state is SupervisorLoading && state.loadingMessage == null; // Loading genérico para ações
                  ...
                  return Row(
                    ...
                    children: [
                      AppButton(
                        text: 'Rejeitar',
                        onPressed: isLoadingAction ? null : () => _showRejectionReasonDialog(context, log.id),
                        type: AppButtonType.text,
                        foregroundColor: theme.colorScheme.error,
                        isLoading: isLoadingAction && _supervisorBloc.state is SupervisorLoading, // Verifica se este log específico está a ser processado
                      ),
                      ...
                      AppButton(
                        ...
                        isLoading: isLoadingAction && _supervisorBloc.state is SupervisorLoading,
                      ),
                    ],
                  );
                },
              ...
              Row(
                ...
                children: [
                  ...
                  Text('Aprovado', style: TextStyle(color: AppColors.success, fontWeight: FontWeight.bold)),
                  if(log.supervisorId != null)
                    Text(' por ${_getSupervisorName(log.supervisorId!)}', style: theme.textTheme.bodySmall?.copyWith(color: theme.hintColor)), // TODO: Implementar _getSupervisorName
                ],
              )
          ],
        ),
