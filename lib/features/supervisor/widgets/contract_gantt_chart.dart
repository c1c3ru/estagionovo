...
import '../../../../data/models/enums.dart'; // Para ContractStatus
...
class ContractGanttChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime overallStartDate = contracts.isNotEmpty
        ? contracts.map((c) => c.startDate).reduce((a, b) => a.isBefore(b) ? a : b)
        : DateTime.now();
    ...
    if (overallStartDate == overallEndDate) { // Evitar divisão por zero se todas as datas forem iguais
        overallEndDate = overallEndDate.add(const Duration(days: 1));
    }
    ...
    final int totalDurationInDays = overallEndDate.difference(overallStartDate).inDays;
    if (totalDurationInDays <= 0) {
      return const Center(child: Text("Intervalo de datas inválido para o gráfico Gantt."));
    }
    ...
    final double chartAreaWidth = MediaQuery.of(context).size.width - nameColumnWidth - 48; // 48 para paddings gerais
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          padding: const EdgeInsets.only(left: nameColumnWidth + 16.0, right: 16.0, bottom: 8.0),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(DateFormat('dd/MM/yy').format(overallStartDate), style: theme.textTheme.bodySmall),
              Text('Duração Total: $totalDurationInDays dias', style: theme.textTheme.bodySmall?.copyWith(fontWeight: FontWeight.bold)),
              Text(DateFormat('dd/MM/yy').format(overallEndDate.subtract(const Duration(days:1))), style: theme.textTheme.bodySmall),
            ],
          ),
        ),
        const Divider(height: 1),
        Expanded( // Para permitir que o ListView ocupe o espaço restante
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final contractStartOffsetDays = contract.startDate.difference(overallStartDate).inDays;
              final contractDurationDays = contract.endDate.difference(contract.startDate).inDays.clamp(0, totalDurationInDays); // Clamp para não exceder

              // Calcula as larguras proporcionais para a barra
              final double startPadding = (contractStartOffsetDays / totalDurationInDays) * chartAreaWidth;
              final double barWidth = (contractDurationDays / totalDurationInDays) * chartAreaWidth;
                    ...
                    Expanded(
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: theme.colorScheme.surfaceVariant.withAlpha(80), // Fundo da linha
                          borderRadius: BorderRadius.circular(4),
                        ),
                        child: Stack(
                          children: [
                            Positioned(
                              left: startPadding.isFinite && !startPadding.isNaN ? startPadding.clamp(0, chartAreaWidth) : 0,
                              top: 0,
                              bottom: 0,
                              child: Tooltip(
                                message:
                                ...
                                child: Container(
                                  width: barWidth.isFinite && !barWidth.isNaN ? barWidth.clamp(4, chartAreaWidth) : 4.0, // Mínimo de 4px de largura
                                  decoration: BoxDecoration(
                                    color: _getContractStatusColor(contract.status, context),
                                    borderRadius: BorderRadius.circular(4),
                                    border: Border.all(color: theme.dividerColor.withAlpha(100), width: 0.5)
                                  ),
